...
Fixpoint eval_dual (e : Expr) (x : D) : D :=
    match e with
    | Const c   => inj c
    | Var       => x
...
Fixpoint deriv (e : Expr) : Expr :=
    ...
    | Mul e1 e2 => Add (Mul (deriv e1) e2) (Mul e1 (deriv e2))
    | Opp e1    => Opp (deriv e1)
    | Div e1 e2 =>
        (* (e1' e2 - e1 e2') / e2^2 *)
        Div (Sub (Mul (deriv e1) e2) (Mul e1 (deriv e2)))
            (Mul e2 e2)
    end.

(* Well-definedness: Every denominator is nonzero *)
Fixpoint defined_at (e : Expr) (x : S) : Prop :=
    match e with
    | Const _   => True
    | Var       => True
...
(* Correctness
   ...
   After evaluating [e] over the duals at [a + eps], its real part
   is [eval e a], and its dual part is [eval (deriv e) a]. *)
Theorem eval_dual_correct :
    forall e a,
        defined_at e a ->
        real (eval_dual e (a, 1%S)) = eval e a /\
        du (eval_dual e (a, 1%S))   = eval (deriv e) a.
Proof.
    induction e; intros a Hdef; cbn in Hdef.
    - (* Const c *)
    ...
    - (* Add e1 e2 *)
      ...
      destruct (IHe1 a H1) as [Hr1 Hd1].
      destruct (IHe2 a H2) as [Hr2 Hd2].
