...
Module QcField <: DField.
    ...
    Definition inv := Qcinv.
    Definition div := Qcdiv.
    Infix "/" := div (at level 40, left associativity) : S_scope.
    Notation "/ x" := (inv x) (at level 35, right associativity) : S_scope.

    Lemma div_def : forall x y, (x / y = x * inv y)%S.
    Proof. intros. unfold div, inv, mul, Qcdiv. reflexivity. Qed.

    Lemma mul_inv_r : forall x, (x <> 0)%S -> (x * inv x = 1)%S.
    Proof.
        intros x Hx. unfold mul, inv, one.
        now apply Qcmult_inv_r.
    Qed.

    Lemma inv_0 : (inv 0 = 0)%S.
    Proof. reflexivity. Qed.

    Lemma ft : field_theory zero one add mul sub opp div inv eq.
    Proof.
        constructor.
        - exact rt.
        - unfold one, zero. discriminate.
        - intros. unfold div, inv. reflexivity.
        - intros p Hp. unfold mul, inv, one. apply Qcmult_inv_l.
          unfold zero in Hp. exact Hp.
    Qed.
End QcField.
